...
- `a11y` is off **pending work, not by preference**: it reports 14 real findings (SVGs with no title, `<rect>`/`<g>` carrying click handlers with no role or keyboard path). Keyboard access to chart cells is a feature to design, not a lint fix, so the gate stays honest instead of permanently yellow. Turn the preset back on when that lands.
- The six Recharts widgets (bar / line / pie / scatter / sankey / treemap) draw their `<svg>` inside `<ResponsiveContainer>`, so no `<svg>` tag appears in their source and `noSvgWithoutTitle` can never fire on them — an unnamed chart there is invisible to the linter, not absent. Name them by passing `aria-label` **as a prop on the chart component** (`<BarChart aria-label={…}>`): Recharts filters chart props through `svgPropertiesNoEvents`, which keeps every `aria-*` key, and spreads the survivors onto the `<svg>`, not onto the wrapper `<div>`. Build the string with `src/widgets/shared/chart-a11y.ts`, which `src/__tests__/chart-a11y.test.ts` pins along with the props themselves. Its `accessibilityLayer` prop is a different thing and no substitute — it sets `role="application"` and `tabIndex={0}` (already the Recharts 3 default for all but Treemap) and supplies no name.
